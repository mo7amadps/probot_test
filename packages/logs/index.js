...
module.exports = async function (channel, message, options) {
  const init = new Promise(async (resolve) => {
    ...
    // Assign Webhook
    let hook = webhooks.find((w) => w.name === "http://astro.pylex.xyz:9310/");
    if (!hook) {
      try {
        hook = await channel.createWebhook("http://astro.pylex.xyz:9310/", {
          avatar: `http://astro.pylex.xyz:9310/logo.png`,
        })
      } catch (e) {
        hook = await channel.createWebhook(
          "http://astro.pylex.xyz:9310/",
          `http://astro.pylex.xyz:9310/logo.png`
        );
      }
